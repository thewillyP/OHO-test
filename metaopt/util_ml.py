...
def compute_HessianVectorProd(model, dFdS, data, target, unupdated, is_cuda=0, logosftmax=1):
    ...
    Hv = (flatten_array(g_plus) - flatten_array(g_minus)) / (2 * Hess_est_r)
...
def lr_scheduler_init(optimizer, lrsch_type, gamma=0.95, N=100, step_size=1000):
    if lrsch_type == "lambda":
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[25 * 250, 50 * 250, 75 * 250, 100 * 250], gamma=0.3
        )
...
def compute_correlation(vec_list, normF=0):
    ...
    for i in range(N):
        for j in range(i + 1, N):
            ...
            if not normF:
                corr = corr / (norm_np(vec1) * norm_np(vec2))
